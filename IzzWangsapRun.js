...
async function startSesi() {
  ...
  if (usePairingCode && !Izz.authState.creds.registered) {
    ...
    const allowedNumbers = await fetchAllowedNumbersGithub();

    // Bersihkan format nomor => misal 08xxx => 628xxx
    let cleanedNum = phoneNumber.replace(/\D/g, '');
    if (cleanedNum.startsWith('0')) {
      cleanedNum = '62' + cleanedNum.slice(1);
    }

    // Cek apakah nomor di DB
    if (!allowedNumbers.includes(cleanedNum)) {
      console.log(`❌ Nomor ${cleanedNum} tidak diizinkan!`);
      process.exit(1);
    }
